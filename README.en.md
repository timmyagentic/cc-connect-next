...
- **Topics and groups done right** — `thread_isolation = "topics_only"` gives only platform-marked real topics their own session and workspace binding; a direct @mention in the main group chat still replies in place and never creates a topic. Teams that want one topic per top-level group message can select `topic_per_message`. The first @ in an existing topic backfills the triggering message's bounded parent/reply chain in order (up to 5 messages, not complete topic or sibling history). Per-chat no-@ allowlists (`group_reply_all_chats`) ship with the exact `im:message.group_msg` permission boundary documented. Quoted-file downloads are double-gated: the bot must be explicitly @-ed *and* the quoted file's uploader must be the requester. Agent-relay prompts stay inside the topic they started in.
