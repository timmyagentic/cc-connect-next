...
- Keep update checks and release downloads on `timmyagentic/cc-connect-feishu-plus`;
  falling back to the upstream release feed would silently remove Plus features.
- Run `npm run check --prefix npm` for bootstrap changes.
- Do not enable npm or modified-binary publication until signed assets, automatic
  rollback, and the licensing gate in `docs/upstream-baseline.md` are resolved.
