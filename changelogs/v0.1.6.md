...
- **不需要测试应用的一键生产迁移。** 当官方 CC Connect 与 cc-connect-next
  ...
  CLI 私聊唯一或显式飞书/Lark 操作者；歧义和发送失败不会误发群聊或回滚迁移。
...
- **One-command production migration needs no test app.** Startup refuses an
  ...
  Next service, then stops/disables official, final-syncs, and starts Next. The
  CLI privately notifies one unique or explicit Feishu/Lark operator; ambiguity
  and send failure never fall back to a group or roll back successful migration.
