...
//     the shape official CC Connect configs may use for args with spaces
...
func toStringArray(v any) ([]string, bool) {
	...
	case []any:
		...
		for _, item := range arr {
			s, ok := item.(string)
			if !ok {
				// Mixed-type or non-string arrays are an invalid cmd shape;
				// let the caller fall through rather than silently drop
				// part of the user's config.
				return nil, false
			}
			out = append(out, s)
		}
...
// TOML allows sparse inline arrays (`["cli", "", "--flag"]`); the empty slots
// are treated as unset rather than spawning the CLI with an empty argv entry.
