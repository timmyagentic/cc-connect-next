...
func TestCUJ_H4_FeishuTopicsKeepWorkspaceBindingsIsolated(t *testing.T) {
	...
	// Topic A gets its own binding. Topic B first inherits the chat default,
	// then gets an independent override.
	sendTopicCommand("om_root_a", "/workspace bind workspace-a")
	sendTopicCommand("om_root_b", "/workspace")
	if got := lastReply(); !strings.Contains(got, defaultWorkspace) {
	...
	if got := lastReply(); !strings.Contains(got, workspaceA) {
		t.Fatalf("topic A workspace reply = %q, want %q", got, workspaceA)
	}
	sendTopicCommand("om_root_b", "/workspace")
	if got := lastReply(); !strings.Contains(got, workspaceB) {
		t.Fatalf("topic B workspace reply = %q, want %q", got, workspaceB)
	}

	// Unbinding A restores the inherited chat default without changing B.
	sendTopicCommand("om_root_a", "/workspace unbind")
	sendTopicCommand("om_root_a", "/workspace")
	if got := lastReply(); !strings.Contains(got, defaultWorkspace) {
		t.Fatalf("topic A did not fall back to the chat default: %q", got)
	}
	sendTopicCommand("om_root_b", "/workspace")
	if got := lastReply(); !strings.Contains(got, workspaceB) {
...
func TestCUJ_J1_FeedbackExplicitTriggerSubmitsDirectly(t *testing.T) {
	...
	for _, mode := range []string{"shared session", "multi workspace", "forced work_dir"} {
		t.Run("error offer owner/"+mode, func(t *testing.T) {
			env := newCUJEnv(t)
			if mode != "shared session" {
				workspace := normalizeWorkspacePath(filepath.Join(env.tempDir, "workspace"))
				if err := os.MkdirAll(workspace, 0o755); err != nil {
					t.Fatal(err)
				}
				if mode == "multi workspace" {
					env.engine.SetMultiWorkspace(env.tempDir, filepath.Join(env.tempDir, "bindings.json"))
					env.engine.workspaceBindings.Bind("project:test", "test:shared-topic", "shared-topic", workspace)
				} else {
					env.engine.workspacePool = newWorkspacePool(DefaultWorkspaceIdleTimeout)
					env.engine.bindSendWorkDir("test:shared-topic", workspace)
				}
				ws := env.engine.workspacePool.GetOrCreate(workspace)
				ws.mu.Lock()
				ws.agent = env.agent
				ws.sessions = NewSessionManager(filepath.Join(env.tempDir, "workspace-sessions.json"))
				ws.mu.Unlock()
			}
			env.engine.SetFeedbackConfig(true, "https://relay.example/v1/feedback")
			...
			send := func(user, content string) {
				env.engine.ReceiveMessage(env.plat, &Message{
					...
					Content: content, ReplyCtx: "shared-reply", ChannelKey: "shared-topic",
				})
			}
			send("owner", "hello")
