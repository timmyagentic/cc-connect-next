...
- **话题与群聊做对了** —— `thread_isolation` 给每个话题独立的工作区绑定；在已有话题里第一次 @ 机器人会按顺序回填话题上下文。`group_reply_all_chats` 支持按群指定免 @ 名单，并把 `im:message.group_msg` 权限的确切边界写清楚。引用文件下载有双重门禁：必须显式 @ 机器人，且被引用文件的上传者就是当前提问的人。话题里发起的 Relay 提示留在原话题，不泄漏到群根会话。
