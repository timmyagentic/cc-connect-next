...
This guide walks you through connecting **cc-connect-next** to Discord, so you can chat with your local Claude Code via a Discord bot.
...
2. Enter an application name (e.g. `cc-connect-next`)
...
## Step 5: Configure cc-connect-next
...
> cc-connect-next automatically configures the required Intents (MESSAGE_CONTENT, GUILD_MESSAGES, DIRECT_MESSAGES).
> With `thread_isolation = true`, cc-connect-next creates or reuses a Discord thread for each session and routes follow-up messages by thread channel ID.
...
```
level=INFO msg="discord: connected" bot=cc-connect-next#0000
...
| **Message limit** | 2000 characters per message (auto-split by cc-connect-next) |
