...
Feishu topic isolation is deliberately **not** an upgrade-time behavior change. The canonical string modes are `off`, `topics_only`, and `topic_per_message`. An omitted key or legacy boolean `false` maps to `off`; legacy `true` maps to `topic_per_message`, preserving the historical behavior that promotes every top-level group message into an isolated topic. Only a newly generated Starter config, or an operator who explicitly accepts the recommended Feishu profile, receives `thread_isolation = "topics_only"`: real Feishu topics whose events carry `thread_id` are isolated, while ordinary main-chat messages and non-topic replies retain the per-user/channel session boundary and do not create topics.
