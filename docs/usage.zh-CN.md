...
```text
/feedback <描述>   # 生成并展示完整脱敏草稿
/feedback confirm  # 提交这份精确的待确认草稿
/feedback cancel   # 取消，且不发出请求
```

daemon 也会主动准备预览。飞书等卡片平台会完整显示描述、最近错误、能力缺口和
白名单环境字段，并提供【反馈给作者】【忽略】按钮；文本平台显示相同字段以及
confirm/cancel 命令。回合失败预览按会话节流，不支持的配置键集合跨重启只提醒一次。

`/feedback` 本身绝不提交。凭据、会话/用户 ID、邮箱和文件路径会在草稿产生前完成
脱敏与限长；只有用户执行独立操作后，同一份待确认 Foundation Draft 才能变成 opaque
