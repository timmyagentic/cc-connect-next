...
daemon 也会主动提示。在支持卡片的平台（飞书）上会把问题梳理好并直接询问
「要反馈给作者吗？」，附【反馈给作者】【忽略】两个按钮——同意只需点一下，
不用学任何命令。回合失败后的询问每个会话每 10 分钟至多一次；启动时对无法
消费的配置项每组键只询问一次。无按钮的平台回退为一行 `/feedback` 提示。

发送命令即提交——调用 `/feedback` 本身就是同意。凭据、会话/用户 ID、文件路径
会自动脱敏；自动生成的环境信息只包含版本、操作系统/架构和 Agent 类型。如需整体
关闭，在 config.toml 设置 `[feedback] enabled = false`。
