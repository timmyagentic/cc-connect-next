...
5. 未匹配到的 `@xxx` 保留原文不处理
6. 行内代码、围栏代码块、缩进代码块和反斜杠转义的 `@xxx` 视为示例，保持字面量且不触发通知；答案中若已经带有原生 `<at ...>` 标记，也不会被视为本次解析成功的通知目标，不论 Rich 或 Legacy 发送路径都不会因此强制切换为可触发通知的文本消息
...
从飞书话题发起 Agent Relay 时，请求与响应的可见性提示会通过话题专用回复路径继续留在同一话题，不再泄漏到群聊根会话；即使普通回答配置为 `reply_to_trigger = false`，这条路由也不受影响。非话题会话和其他平台维持原有路由。
