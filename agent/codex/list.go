...
func parseCodexSessionFile(path, filterCwd string) *core.AgentSessionInfo {
	...
	for scanner.Scan() {
		...
		switch entry.Type {
		case "session_meta":
			// A subagent rollout may embed a copy of its parent's
			// session_meta after its own; only the first one identifies
			// the transcript.
			if sessionID != "" {
				continue
			}
...
// internal subagent thread rather than a top-level user session. Top-level
// sessions carry a string source ("cli", "exec", "vscode", …), which fails
// the object unmarshal and is therefore never treated as a subagent.
