...
func writeCodexRollout(t *testing.T, dir, name, sessionID, workDirJSON, source string, withPrompt bool) {
	t.Helper()
	body := `{"type":"session_meta","payload":{"id":"` + sessionID + `","cwd":` + workDirJSON + `,"source":` + source + `}}` + "\n"
	if withPrompt {
		body += `{"type":"response_item","payload":{"role":"user","content":[{"type":"input_text","text":"fix the login bug"}]}}` + "\n"
	}
...
func TestAgentListSessions_ExcludesSubagentRollouts(t *testing.T) {
	...
	writeCodexRollout(t, sessionsDir, "rollout-top-level.jsonl", "top-level", string(workDirJSON), `"vscode"`, true)
	writeCodexRollout(t, sessionsDir, "rollout-subagent.jsonl", "subagent", string(workDirJSON),
		`{"subagent":{"thread_spawn":{"parent_thread_id":"top-level"}}}`, true)
...
func TestAgentListSessions_ExcludesSubagentRolloutWithCopiedParentMeta(t *testing.T) {
	...
	// A subagent rollout can embed a copy of its parent's session_meta after
	// its own; only the first meta line identifies the transcript.
...
func TestAgentListSessions_LongThreadNameTruncated(t *testing.T) {
	...
	writeCodexRollout(t, sessionsDir, "rollout-session.jsonl", sessionID, string(workDirJSON), `"vscode"`, false)
	...
	if sessions[0].Summary != want {
		t.Fatalf("ListSessions()[0].Summary = %q, want truncated thread name %q", sessions[0].Summary, want)
	}
}
