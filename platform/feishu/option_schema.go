...
func feishuConfigOptions(defaultDomain string) []core.ConfigOption {
	...
	for i := range options {
		...
		switch option.Key {
		case "allow_chat":
			option.Default = "empty / allow all chats"
		...
		case "encrypt_key":
			option.Default = "unset / WebSocket mode"
		...
		case "group_reply_all_chats":
			...
			option.Default = "empty / use group_reply_all"
		...
		case "reply_to_trigger":
			option.Default = "true"
			option.Description = "Reply using the triggering message as the target. When false, ordinary replies are created without quoting it; replies inside a real topic still target that topic's thread_id to preserve topic locality."
			option.DescriptionZH = "以触发消息作为回复目标。设为 false 时普通回复不再引用该消息；真实话题内的回复仍会指向该话题的 thread_id，以保持话题归属。"
		...
		case "thread_isolation":
			...
			option.Default = "off when omitted; new Starter/recommended profile writes topics_only; legacy true maps to topic_per_message"
