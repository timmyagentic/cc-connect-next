...
// This opt-in test installs current public Stable and Beta releases over disposable
// executable. It never touches the running daemon, config, or installed binary.
func TestLiveUpdateServiceAgainstPublishedReleases(t *testing.T) {
	...
	for _, channel := range []updatechannel.Channel{updatechannel.Stable, updatechannel.Beta} {
		t.Run(string(channel), func(t *testing.T) {
			target := filepath.Join(t.TempDir(), ProductName)
			if err := os.WriteFile(target, versionScript("v0.2.0"), 0o755); err != nil {
				t.Fatal(err)
			}
			service, err := NewUpdateService(UpdateConfig{CurrentVersion: "v0.2.0", Channel: channel, ExecutablePath: target})
			if err != nil {
